      ...
      <div className="p-5 space-y-4">
            ...
            {recent.length > 0 ? recent.map(exec => (
              <div key={exec.id} className="flex items-center gap-2.5 py-1.5 px-2.5 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                ...
                <span className="text-xs text-slate-400 dark:text-slate-500 flex-shrink-0">
                  {exec.timestamp ? new Date(exec.timestamp).toLocaleTimeString('en-CA', { hour: '2-digit', minute: '2-digit', timeZone: 'Etc/GMT+5' }) : '—'}
                </span>
              </div>
            )) : (
        ...
        <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-700">
          <div className="flex items-center gap-1.5 text-xs text-slate-400 dark:text-slate-500">
            <Clock size={12} />
            {lastRun
              ? `Last run: ${new Date(lastRun).toLocaleDateString('en-CA', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit', timeZone: 'Etc/GMT+5' })}`
              : 'No runs yet'}
          </div>
          {config.webhook && (
            <a
